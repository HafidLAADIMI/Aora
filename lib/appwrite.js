...
export const config = {
  ...
  videosCollectionId: "66dee73500055b9324bd",
...
export const savePost = async (postId,userId) => {
  try {
    ...
    const video=await database.getDocument(config.databaseId,config.videosCollectionId,postId);
    console.log(video)
    const likedBy=video.likedBy || [];
    console.log(likedBy);
    const isAlreadySaved=likedBy.includes(userId);
    const updateLikedBy=isAlreadySaved?likedBy.filter((id)=>id!==isAlreadySaved):[...likedBy,userId];
